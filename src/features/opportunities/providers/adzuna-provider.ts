...
export class AdzunaProvider implements IOpportunityProvider {
	...
	async searchJobs(profile: any): Promise<NormalizedOpportunity[]> {
		try {
			...
			const synonym = targetCareer.toLowerCase().includes("software engineer") ? "Developer" : "Software Engineer";
			...
			for (const q of queries) {
				...
				try {
					...
					console.log(`[PROVIDER] Adzuna | Query: "${q.query}" (${q.name}) | Fetched: ${results.length}`);
