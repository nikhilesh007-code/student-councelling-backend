...
export class DashboardService {
  async getDashboardSummary(userId: string) {
    ...
    try {
      ...
      const studyMinutes = completedTasksArr.reduce((acc: number, t: any) => acc + (t.estimatedMinutes || 0), 0);
    ...
    const careerMatchScore = recommendedCareers.length > 0 ? recommendedCareers[0].matchPercentage : null;
    ...
    let roadmapTotal = roadmapPhases.length;
    ...
    let overallProgress = Math.round((profileCompletion + readinessScore + roadmapProgress) / 3) || 0;
    ...
    // Merge tasks
    let upcomingTasks = [...studyStats.upcomingTasks, ...roadmapTasks].slice(0, 5);
    ...
    return {
      ...
      onboarding: {
        ...
        score: Math.round(((profileCompletion >= 80 ? 20 : 0) + (recommendedCareers.length > 0 ? 20 : 0) + (roadmapPhases.length > 0 ? 20 : 0) + (!!skillGapData?.readinessScore ? 20 : 0) + (hasPlannerTasks ? 20 : 0)))
      }
    };
  }
}
