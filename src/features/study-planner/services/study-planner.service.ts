...
import { GeneratedDayPlan, studyPlannerAiService } from "./study-planner-ai.service";
...
export class StudyPlannerService {
	...
	async generatePlan(userId: string, prefsPayload: any) {
		...
		if (replaceExisting) {
			...
			const orphanTasks = await prisma.studyTask.findMany({
				where: { userId, planId: null, isArchived: false },
			});
	...
	async archivePlan(userId: string, planId: string) {
		...
		await prisma.$transaction([
			...
			...(taskIds.length > 0
				? [prisma.notification.deleteMany({ where: { userId, taskId: { in: taskIds } } })]
				: []),
		]);
