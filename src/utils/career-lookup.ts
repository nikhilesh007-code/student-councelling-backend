...
export async function resolveCareer(careerGoalInput: string | null, userSkills: string[] = []) {
	...
	if (!career) {
		if (userSkills.length > 0) {
			console.log(
				`[Warning] Career goal '${originalInput}' not found. Computing dynamic fallback based on skills.`,
			);
			let bestScore = -1;
			let bestCareer = null;
			...
			if (bestCareer) {
				console.log(
					`[Info] Dynamically resolved fallback career to '${bestCareer.name}' with score ${bestScore}%`,
				);
				career = bestCareer;
			}
		}
		...
		if (!career) {
			console.warn(
				`[Warning] Career goal '${originalInput}' not found and no skills matched. Falling back to 'Developer'.`,
			);
